/* ================================================================
   SOCIAL ICON — the official mark for each platform the site links
   to, drawn here as inline SVG.

   WHY THESE ARE HAND-DRAWN
   ----------------------------------------------------------------
   lucide-react dropped its brand set in v1, and the Bedar design
   system ships no social marks. The footer previously stood in with
   the platform's first LETTER in a circle ("F", "I", "L", "X"),
   which is what the brief asks to replace with the real marks.

   Adding a brand-icon dependency for four glyphs would be ~40 kB for
   four paths, on a site whose whole architecture note is about
   keeping weight off the public bundle. Four paths inline is a few
   hundred bytes and ships in the component chunk.

   Each path is the platform's own published glyph geometry on a
   24×24 grid, filled rather than stroked — brand marks are solid
   shapes, and stroking them at lucide's 1.75 weight makes them read
   as sketches of the logo rather than as the logo.

   `currentColor` throughout, so the chip's hover colour drives the
   mark with no per-icon rule.

   `KNOWN` is keyed by the account `id` in `content/site.js`. An id
   with no mark here falls back to `Globe`, so adding a fifth account
   in the dashboard shows a sane generic link chip instead of
   crashing or rendering an empty circle.
   ================================================================ */

import { Globe } from 'lucide-react';

/** Shared wrapper: 24-grid, fills with the inherited colour. */
function Glyph({ children, ...rest }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="currentColor"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden="true"
      focusable="false"
      {...rest}
    >
      {children}
    </svg>
  );
}

function FacebookMark(props) {
  return (
    <Glyph {...props}>
      <path d="M24 12.07C24 5.4 18.63 0 12 0S0 5.4 0 12.07C0 18.1 4.39 23.1 10.13 24v-8.44H7.08v-3.49h3.05V9.41c0-3.02 1.79-4.69 4.53-4.69 1.31 0 2.68.24 2.68.24v2.97h-1.51c-1.49 0-1.96.93-1.96 1.89v2.25h3.33l-.53 3.49h-2.8V24C19.61 23.1 24 18.1 24 12.07Z" />
    </Glyph>
  );
}

function InstagramMark(props) {
  return (
    <Glyph {...props}>
      <path d="M12 2.16c3.2 0 3.58.01 4.85.07 1.17.05 1.8.25 2.23.41.56.22.96.48 1.38.9.42.42.68.82.9 1.38.16.42.36 1.06.41 2.23.06 1.27.07 1.65.07 4.85s-.01 3.58-.07 4.85c-.05 1.17-.25 1.8-.41 2.23-.22.56-.48.96-.9 1.38-.42.42-.82.68-1.38.9-.42.16-1.06.36-2.23.41-1.27.06-1.65.07-4.85.07s-3.58-.01-4.85-.07c-1.17-.05-1.8-.25-2.23-.41a3.72 3.72 0 0 1-1.38-.9 3.72 3.72 0 0 1-.9-1.38c-.16-.42-.36-1.06-.41-2.23-.06-1.27-.07-1.65-.07-4.85s.01-3.58.07-4.85c.05-1.17.25-1.8.41-2.23.22-.56.48-.96.9-1.38.42-.42.82-.68 1.38-.9.42-.16 1.06-.36 2.23-.41 1.27-.06 1.65-.07 4.85-.07M12 0C8.74 0 8.33.01 7.05.07 5.78.13 4.9.33 4.14.63c-.79.3-1.46.72-2.13 1.38A5.9 5.9 0 0 0 .63 4.14c-.3.76-.5 1.64-.56 2.91C.01 8.33 0 8.74 0 12s.01 3.67.07 4.95c.06 1.27.26 2.15.56 2.91.3.79.72 1.46 1.38 2.13a5.9 5.9 0 0 0 2.13 1.38c.76.3 1.64.5 2.91.56C8.33 23.99 8.74 24 12 24s3.67-.01 4.95-.07c1.27-.06 2.15-.26 2.91-.56.79-.3 1.46-.72 2.13-1.38a5.9 5.9 0 0 0 1.38-2.13c.3-.76.5-1.64.56-2.91.06-1.28.07-1.69.07-4.95s-.01-3.67-.07-4.95c-.06-1.27-.26-2.15-.56-2.91a5.9 5.9 0 0 0-1.38-2.13A5.9 5.9 0 0 0 19.86.63c-.76-.3-1.64-.5-2.91-.56C15.67.01 15.26 0 12 0Zm0 5.84a6.16 6.16 0 1 0 0 12.32 6.16 6.16 0 0 0 0-12.32ZM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8Zm7.85-10.4a1.44 1.44 0 1 1-2.88 0 1.44 1.44 0 0 1 2.88 0Z" />
    </Glyph>
  );
}

function LinkedinMark(props) {
  return (
    <Glyph {...props}>
      <path d="M20.45 20.45h-3.56v-5.57c0-1.33-.02-3.04-1.85-3.04-1.85 0-2.14 1.45-2.14 2.94v5.67H9.35V9h3.41v1.56h.05a3.74 3.74 0 0 1 3.37-1.85c3.6 0 4.27 2.37 4.27 5.46v6.28ZM5.34 7.43a2.07 2.07 0 1 1 0-4.14 2.07 2.07 0 0 1 0 4.14Zm1.78 13.02H3.55V9h3.57v11.45ZM22.22 0H1.77C.79 0 0 .77 0 1.73v20.54C0 23.22.79 24 1.77 24h20.45c.98 0 1.78-.78 1.78-1.73V1.73C24 .77 23.2 0 22.22 0Z" />
    </Glyph>
  );
}

function XMark(props) {
  return (
    <Glyph {...props}>
      <path d="M18.9 1.15h3.68l-8.04 9.19L24 22.85h-7.4l-5.8-7.58-6.64 7.58H.47l8.6-9.83L0 1.15h7.6l5.24 6.93 6.06-6.93Zm-1.29 19.5h2.04L6.48 3.24H4.3l13.31 17.41Z" />
    </Glyph>
  );
}

const KNOWN = {
  facebook: FacebookMark,
  instagram: InstagramMark,
  linkedin: LinkedinMark,
  x: XMark,
  twitter: XMark,
};

/**
 * The mark for a social account.
 *
 * @param {string} id  the account id from `content/site.js`
 */
export function SocialIcon({ id, className }) {
  const Mark = KNOWN[id];
  if (!Mark) return <Globe aria-hidden="true" className={className} strokeWidth={1.75} />;
  return <Mark className={className} />;
}

export default SocialIcon;
